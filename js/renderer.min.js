import{html,render}from"../lit-html/lit-html.js";const $textID="text",$fileID="file",$keyGenID="keygen",$RSA_ID="RSA";let prevTabID=null;const minimise=function(){window.winCtl.min()},maximise=function(){window.winCtl.max()},close=function(){window.winCtl.close()},keyGenInflate=function(){renderTab(keyGen,"keygen","Keyfile generation")},textInflate=function(){renderTab(text,"text","Text cryptography")},fileInflate=function(){renderTab(file,"file","File cryptography")},RSAInflate=function(){renderTab(RSA,"RSA","RSA cryptography")},$=t=>document.getElementById(t),q=t=>document.querySelector(t),store=(t,e)=>localStorage.setItem(t,e),get=t=>localStorage.getItem(t),clrActive=()=>{document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")})},renderTab=(t,e,a)=>{if(e!==prevTabID){if("function"==typeof onStop){console.debug(`<renderTab> Stopping activity ${prevTabID}`);try{onStop()}catch(t){console.warn(`<renderTab> Activity onStop threw an exception:\n\n${t}`)}}q(".titleBar .windowTitle small").textContent=a,render(t(),$("main")),window.mdc.autoInit(),store("lastTab",e),document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")}),console.debug(`<renderTab> Loading activity ${e}`),loadJS(`js/backend/activities/${e}Activity.min.js`,function(){onStart().then(()=>{console.debug(`<renderTab> Loaded activity ${e}`)}).catch(t=>{console.warn(`<renderTab> ${e} activity onStart threw an exception:\n\n ${t}`)})}).catch(t=>{console.debug(`<renderTab> Failed to fetch and execute activity JS script for activity ${e} with error:\n\n${t}`)}),prevTabID=e,$(e).classList.add("active")}},loadJS=async(t,e=function(){console.debug(`Loaded script '${t}`)})=>{const a=document.createElement("script");a.onload=e,a.src=t;try{document.head.appendChild(a)}catch(e){console.error(`Failed to fetch script '${t}' with error: \n\n${e}`)}},page=()=>html`
    <div class="titleBar">
