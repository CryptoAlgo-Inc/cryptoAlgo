import{html,render}from"../lit-html/lit-html.js";const $textID="text",$fileID="file",$keyGenID="keygen",$RSA_ID="RSA";let prevTabID=null;const minimise=function(){window.winCtl.min()},maximise=function(){window.winCtl.max()},close=function(){window.winCtl.close()},keyGenInflate=function(t=!0){renderTab(keyGen,"keygen","Keyfile generation",t)},textInflate=function(t=!0){renderTab(text,"text","Text cryptography",t)},fileInflate=function(t=!0){renderTab(file,"file","File cryptography",t)},RSAInflate=function(t){renderTab(RSA,"RSA","RSA cryptography",t)};document.body.classList.add(window.proInfo.platform());const $=t=>document.getElementById(t),q=t=>document.querySelector(t),store=(t,e)=>localStorage.setItem(t,e),get=t=>localStorage.getItem(t),clrActive=()=>{document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")})},renderTab=(t,e,a,i=!0)=>{if(e===prevTabID)return;if("function"==typeof onStop)try{onStop()}catch(t){}const l=$("main");let s;i&&l.classList.add("faded"),setTimeout(()=>{render(t(),l),i&&l.classList.remove("faded"),setTimeout(()=>{q(".titleBar .windowTitle small").textContent=a},s),window.mdc.autoInit(),store("lastTab",e),document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")}),loadJS(`js/backend/activities/${e}Activity.min.js`,function(){onStart().then(()=>{}).catch(t=>{})}).catch(t=>{}),prevTabID=e,$(e).classList.add("active")},s=i?200:0)},loadJS=async(t,e=function(){})=>{const a=document.createElement("script");a.onload=e,a.src=t;try{document.head.appendChild(a)}catch(t){}},page=()=>html`
    <div class="titleBar">
