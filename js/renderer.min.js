import{html,render}from"../lit-html/lit-html.js";const $textID="text",$fileID="file",$keyGenID="keygen",$RSA_ID="RSA";let prevTabID=null;const minimise=function(){window.winCtl.min()},maximise=function(){window.winCtl.max()},close=function(){window.winCtl.close()},keyGenInflate=function(){renderTab(keyGen,"keygen","Keyfile generation")},textInflate=function(){renderTab(text,"text","Text cryptography")},fileInflate=function(){renderTab(file,"file","File cryptography")},RSAInflate=function(){renderTab(RSA,"RSA","RSA cryptography")},$=t=>document.getElementById(t),q=t=>document.querySelector(t),store=(t,a)=>localStorage.setItem(t,a),get=t=>localStorage.getItem(t),clrActive=()=>{document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")})},renderTab=(t,a,e)=>{if(a!==prevTabID){if("function"==typeof onStop){console.debug(`<renderTab> Stopping activity ${prevTabID}`);try{onStop()}catch(t){console.warn(`<renderTab> Activity onStop threw an exception:\n\n${t}`)}}q(".titleBar .windowTitle small").textContent=e,render(t(),$("main")),window.mdc.autoInit(),store("lastTab",a),document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")}),console.debug(`<renderTab> Loading activity ${a}`),loadJS(`js/backend/activities/${a}Activity.min.js`,function(){onStart().then(()=>{console.debug(`<renderTab> Loaded activity ${a}`)}).catch(t=>{console.warn(`<renderTab> ${a} activity onStart threw an exception:\n\n ${t}`)})}).catch(t=>{console.debug(`<renderTab> Failed to fetch and execute activity JS script for activity ${a} with error:\n\n${t}`)}),prevTabID=a,$(a).classList.add("active")}},loadJS=async(t,a=function(){console.debug(`Loaded script '${t}`)})=>{const e=document.createElement("script");e.onload=a,e.src=t;try{document.head.appendChild(e)}catch(a){console.error(`Failed to fetch script '${t}' with error: \n\n${a}`)}},page=()=>html`
    <div class="titleBar">
