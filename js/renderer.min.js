import{html,render}from"../lit-html/lit-html.js";const $textID="text",$fileID="file",$keyGenID="keygen",$RSA_ID="RSA";let prevTabID=null;const minimise=function(){window.winCtl.min()},maximise=function(){window.winCtl.max()},close=function(){window.winCtl.close()},keyGenInflate=function(t=!0){renderTab(keyGen,"keygen","Keyfile generation",t)},textInflate=function(t=!0){renderTab(text,"text","Text cryptography",t)},fileInflate=function(t=!0){renderTab(file,"file","File cryptography",t)},RSAInflate=function(t){renderTab(RSA,"RSA","RSA cryptography",t)},$=t=>document.getElementById(t),q=t=>document.querySelector(t),store=(t,e)=>localStorage.setItem(t,e),get=t=>localStorage.getItem(t),clrActive=()=>{document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")})},renderTab=(t,e,a,i=!0)=>{if(e===prevTabID)return;if("function"==typeof onStop){console.debug(`<renderTab> Stopping activity ${prevTabID}`);try{onStop()}catch(t){console.warn(`<renderTab> Activity onStop threw an exception:\n\n${t}`)}}const l=$("main");let s;i&&l.classList.add("faded"),setTimeout(()=>{render(t(),l),i&&l.classList.remove("faded"),setTimeout(()=>{q(".titleBar .windowTitle small").textContent=a},s),window.mdc.autoInit(),store("lastTab",e),document.querySelectorAll(".titleBar .titleTabs button").forEach(t=>{t.classList.remove("active")}),console.debug(`<renderTab> Loading activity ${e}`),loadJS(`js/backend/activities/${e}Activity.min.js`,function(){onStart().then(()=>{console.debug(`<renderTab> Loaded activity ${e}`)}).catch(t=>{console.warn(`<renderTab> ${e} activity onStart threw an exception:\n\n ${t}`)})}).catch(t=>{console.debug(`<renderTab> Failed to fetch and execute activity JS script for activity ${e} with error:\n\n${t}`)}),prevTabID=e,$(e).classList.add("active")},s=i?200:0)},loadJS=async(t,e=function(){console.debug(`Loaded script '${t}`)})=>{const a=document.createElement("script");a.onload=e,a.src=t;try{document.head.appendChild(a)}catch(e){console.error(`Failed to fetch script '${t}' with error: \n\n${e}`)}},page=()=>html`
    <div class="titleBar">
...
`;render(page(),document.body),document.title="CryptoAlgo";const lastTab=get("lastTab");switch(lastTab){case"text":textInflate(!1);break;case"file":fileInflate(!1);break;case"RSA":RSAInflate(!1);break;case"keygen":keyGenInflate(!1);break;default:keyGenInflate(!1)}document.querySelectorAll(".buttonHolder button").forEach(t=>{t.addEventListener("mouseover",()=>{document.body.classList.add("macBtn-hover")},!1),t.addEventListener("mouseout",()=>{document.body.classList.remove("macBtn-hover")},!1)});